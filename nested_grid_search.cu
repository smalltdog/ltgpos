 ...
 */
int input_check(unsigned int* nOfSensor, F* sensorLocs, F* sensorTimes, Info_t* info_p, F outAns[5], bool is_3D)
{
...
void nested_grid_search_sph(unsigned int nOfSensor, F* sensorLocs, F* sensorTimes, Info_t* info_p, F outAns[5], bool is_3D)
{
    ...
    if (input_check(&nOfSensor, sensorLocs, sensorTimes, info_p, outAns, is_3D)) return;

    if (is_3D && gridZSizeFst < 1) {
    ...
    if (DEFAULT_SECOND_SEARCH) {
        ...
        if (is_3D) zz = hSchDomFst[4] + (minIndexFst % gridZSizeFst) * gridInvFst;
        /// 二级搜索范围
 ...
 * @param  schDom           搜索范围，如[28, 32,100, 104，0，0], 要求是6个数，is_3D为true时后两个为低到高
 * @param  is_3D            是否是3d搜索,default false
