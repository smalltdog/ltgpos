...
char* formatRetJsonStr(schdata_t* schdata, cJSON* jarr)
{
    ...
    int num_involved = 0;
    int is_involved[kMaxNumSsrs] = {0};
    F all_dist[kMaxNumSsrs], all_dtime[kMaxNumSsrs];
    F inv_us[kMaxNumSsrs], inv_itdfs[kMaxNumSsrs];
    char* inv_nodes[kMaxNumSsrs];
