...
__global__ void calGirdGoodness2d_G(info_t* info_p, int num_sensors, F* sensor_locs, F* sensor_times)
{
    F basetime, dtime, err = 0;
    F x = info_p->sch_dom[0] + info_p->grid_inv * threadIdx.x;
    F y = info_p->sch_dom[2] + info_p->grid_inv * blockIdx.x;
    int num_dims = info_p->is3d ? 3 : 2;
    ...
    err /= num_sensors - 2;
    info_p->outs_d[blockIdx.x * blockDim.x + threadIdx.x] = err;
}


__global__ void calGirdGoodness3d_G(info_t* info_p, int num_sensors, F* sensor_locs, F* sensor_times)
{
    F basetime, dtime, err = 0;
    F x = info_p->sch_dom[0] + info_p->grid_inv * threadIdx.x;
    F y = info_p->sch_dom[2] + info_p->grid_inv * blockIdx.x;
    F z = info_p->sch_dom[3] + info_p->grid_inv * blockIdx.y;

    basetime = getGeoDistance3d_D(sensor_locs[0], sensor_locs[1], sensor_locs[2], x, y, z);
    for (int i = 1; i < num_sensors; i++) {
        dtime = getGeoDistance2d_D(sensor_locs[i * 3], sensor_locs[i * 3 + 1], sensor_locs[i * 3 + 2],
                                   x, y, z) / C - basetime - sensor_times[i];
        err += dtime * dtime * 1e6;
    }
    err /= num_sensors - 2;
    info_p->outs_d[blockIdx.y * blockDim.x * gridDim.x +
                   blockIdx.x * blockDim.x + threadIdx.x] = err;
}


// int dump_to_file(F* outs, int grid_sizes[3], char* filename)
// {
//     FILE* fp = NULL;
//     fp = fopen(filename, "w");
//     if (!fp) {
//         fprintf(stderr, "%s (line %d): Failed to open file \"%s\".\n", __FILE__, __LINE__, filename);
//         return 1;
//     }
...
void grid_search(sysinfo_t* sysinfo, int num_sensors, F* sensor_locs, F* sensor_times, F results[5])
{
    ...
    for (int i = 0; i < kNumSearches; i++) {
        ...
        for (int i = 0; i < 3; i++) {
            grid_size[i] = (sch_dom[i * 2 + 1] - sch_dom[i * 2]) / grid_inv + 1;
        }
        ...
        // 3D grid search
        if (info->is3d && i != 0) {
            dims grid(grid_sizes[1], grid_sizes[2]);
            dims block(grid_sizes[0]);
            calGirdGoodness3d_G <<<grid, block>>> (info, num_sensors, sensor_locs_d, sensor_times_d);
        }
        // 2D grid search
        else {
            dim grid(grid_sizes[1]), block(grid_sizes[0]);
        ...
        for (int i = 1; i < grid_size, i++) {
            if (outs_h[i] >= grid_size) continue;
            min_err = outs_h[i];
            min_idx = i;
        }
        // dump_to_file(outs_h, grid_sizes, "./gridres.csv");
    ...
    results[0] = sensor_times[0] - getGeoDistance3d_D(sensorLocs[0], sensorLocs[1], sensorLocs[2]) / C;
    results[3] *= 100;
    results[4] = min_err;
    return;
}
